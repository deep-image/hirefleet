...
document.getElementsByClassName('card-button')[x].href = butturl + paramstr;
$('.item-overlay')[x].href = butturl + paramstr;	
document.getElementsByClassName('card-button')[x].style.color = '#89d12b';
...
brand_id : 1,
vehicle_class_id : $('#class-id').text(),
additional_charges : $('#age-range').val() == '25' ? 1 : '',
coupon_code : ""		    
    }),
    contentType: "application/json",
    dataType: "json",
...
var additelem = '<div class="extras-div"><div class="extras-detail"><div class="extras-text">'+singval.label+'</div><div class="extras-detail-text">'+singval.short_description+'</div></div><label class="w-checkbox checkbox-field-2"><div class="w-checkbox-input w-checkbox-input--inputType-custom checkboxes"></div><input indexatt="'+ elemid  + index + '" id='+ singval.label.replace(/\s+/g, '-') + ' type="checkbox" class="addit-check"  style="opacity:0;position:absolute;z-index:-1"><span class="radio-label w-form-label">'+singval.label+'</span><div class="extras-button-pricing">&pound;'+Number(singval.total_price_with_taxes.amount)+'</div></label></div>'
	$('.extra-js-accordion-body').append(additelem);
}
 callcount == 0 ? addarr.push({id: elemid + index , name: singval.label , price : singval.total_price_with_taxes.amount, currentprice: 0, quan : 0 , display : false, newid : singval.label.replace(/\s+/g, '-') }) :
 addarr[index].price = singval.total_price_with_taxes.amount;
})
...
for(let x=0; x< addarr.length; x++)
  { 
	...
	console.log(addarr[x].newid)     
     $('#' + addarr[x].newid).attr('type') == 'checkbox'	 ?  $('#'+addarr[x].newid).click() : $('#'+addarr[x].id).val(addarr[x].quan);   
     }
   }   	
}	
